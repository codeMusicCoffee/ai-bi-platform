...
# 五、AI 开发自检流程 (Self-Correction Workflow)

作为 AI 助手，在编写或修改任何前端组件代码前，必须严格执行以下“自检三部曲”，确保输出质量：

### 第一步：规范对齐 (Pre-coding Check)

- [ ] **Token 对齐**：是否已经阅读并确认了本文件 **“零”** 章节中的颜色、圆角和字体数值？
- [ ] **组件复用**：当前任务是否涉及表格、搜索或表单？如果是，必须优先复用 `SealedTable`、`SealedSearch` 和 `SealedForm`。

### 第二步：逻辑实现 (Implementation)

- [ ] **最小侵入**：是否遵循了 `MEMORY[user_global]` 规则？禁止删除原逻辑，禁止“顺手美化”。
- [ ] **按钮规范**：按钮类型和样式是否与 **“一”** 章节的示例一一对应（尤其是取消按钮的灰色边框）？

### 第三步：视觉复核 (Visual Double-check)

- [ ] **硬编码检测**：代码中是否出现了 `blue-500`、`indigo-600` 等 Tailwind 默认色名？如果有，必须立即纠正为精确色值（如 `#306EFD`）。
- [ ] **细节还原**：
  - 容器圆角是否为 `rounded-[12px]`？
  - 组件圆角是否为 `rounded-[6px]`？
  - 操作文字是否为 `text-[13px]`？
- [ ] **交互补充**：所有的 DropdownMenuItem、表单按钮是否都添加了 `cursor-pointer` 和 `focus:bg-blue-50`？

---

**💡 提示：只有当以上所有检查项目均标记为 [x] 时，方可认为代码符合本项目的高级前端工程标准。**
